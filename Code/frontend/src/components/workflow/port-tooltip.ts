...
const TYPE_HELP: Record<string, string> = {
  ...
  'value:boolean': '开关参数（是/否）。可从「开关」节点连入；未连接时使用节点上的控件默认值。',
  'data:source':
  ...
  'data:timeseries': '时间序列 .mat。由「时间序列合成」产出，供批量反演 / 散射约束产品反演使用。',
...
const NAME_HELP: Record<string, string> = {
  ...
  bbox: '连接「空间范围」或「地图视口」。表示本次运行的 AOI；与数据源上声明的原生空间字段是两回事。',
  viewport_bbox:
  ...
  provider_id:
    '钉选天气数据源（open-meteo-online / open-meteo-local / weatherapi / openweather）。留空则按自动优先级选择。',
