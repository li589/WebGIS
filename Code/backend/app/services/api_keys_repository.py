...
class ApiKeysRepository:
    ...
    def _archive_current_locked(
        ...
        *,
        ...
        row = conn.execute(
            "SELECT key_value_encrypted, key_iv, updated_at, created_at FROM api_keys WHERE key_name=?",
            (key_name,),
        ).fetchone()
        if not row:
            return
        version_created_at = row["updated_at"] or row["created_at"] or superseded_at
        conn.execute(
            """
    ...
    def upsert_key(
        ...
        *,
        ...
        """新增或更新 API Key。值变化时归档旧值到 history 并重置过期测试状态。"""
    ...
    def delete_key(self, key_name: str, *, purge_history: bool = True) -> bool:
        with self._connect() as conn:
            cur = conn.execute("DELETE FROM api_keys WHERE key_name=?", (key_name,))
            if purge_history:
                conn.execute(
                    "DELETE FROM api_key_history WHERE key_name=?", (key_name,)
                )
            conn.commit()
