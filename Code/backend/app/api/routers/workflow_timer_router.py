...
- GET    /workflow-timers                   列出定时器（仅管理员）
- POST   /workflow-timers                   创建定时器（仅管理员）
...
鉴权（2026-09）：定时器创建/改删/手动执行仅 **admin 会话**。
Celery Beat tick 不经本路由，由已创建的定时器驱动。
"""
...
def create_timer(
    ...
    cred: CredentialContext = Depends(require_admin),
) -> dict[str, Any]:
    """创建定时器（仅管理员；记录归属 owner_user_id）。
