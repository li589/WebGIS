...
class WeatherApiProvider(WeatherProvider):
    ...
    DESCRIPTION = (
        "WeatherAPI.com 商业天气 API。覆盖 catalog 全部图层："
        "近地面真值网格、高度层近地面外推、气压层稀疏不可用提示。"
        "需在设置中配置 API Key 并启用。"
    )
    VERSION = "1.0.0"
    ...
    def apply_config(self, config: dict[str, Any]) -> None:
        ...
        logger.info(
            "WeatherApiProvider.apply_config: base_url=%s has_key=%s",
            self._base_url,
            bool(self._api_key),
        )
    ...
    def fetch_point_forecast(
        ...
        *,
        ...
        if layer_id not in COMMERCIAL_LAYER_IDS:
            raise ValueError(f"WeatherAPI does not support layer '{layer_id}'")
        if layer_id in PRESSURE_LAYER_IDS:
            # No true pressure-level winds on WeatherAPI — empty OM-style point.
    ...
    def fetch_grid_forecast(
        ...
        *,
        ...
        max_points = 36 if layer_id in HEIGHT_LAYER_IDS else 64
        ...
        for lat in lats:
            for lon in lons:
                try:
                    ...
                    logger.warning(
                        "WeatherAPI grid sample failed at %.3f,%.3f: %s", lat, lon, exc
                    )
    ...
    def _map_forecast_payload(
        self, raw: dict[str, Any], *, forecast_hours: int
    ) -> dict[str, Any]:
        payload = build_empty_om_point_payload(
            timezone=(raw.get("location") or {}).get("tz_id")
        )
