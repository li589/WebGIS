...
WEATHER_LAYER_SPECS: dict[str, WeatherLayerSpec] = {
    ...
    "wind-field-80m": WeatherLayerSpec(
        ...
        # 含 10m：ECMWF 等模型无 80m 真值时由 power-law 外推填入
        current_fields=(
            "wind_speed_80m",
            "wind_direction_80m",
            "wind_speed_10m",
    ...
    "precipitation": WeatherLayerSpec(
        ...
        notes=(
            "Soft heatmap over masked rain cells for Windy-like precip bloom.",
            "Frontend converts polygon cells to point centroids for MapLibre heatmap.",
        ),
    ),
    ...
    "cloud-cover": WeatherLayerSpec(
        ...
        primary_label="Cloud Cover",
        unit_label="%",
        summary_template="Current cloud cover is {value} {unit}.",
        current_fields=("cloud_cover", "weather_code", "visibility"),
        hourly_fields=("cloud_cover", "precipitation", "temperature_2m"),
        legend_ticks=(0, 20, 40, 60, 80, 100),
        notes=(
            "Total cloud cover percentage from Open-Meteo.",
            "Commercial providers do not expose this as a dedicated tile layer.",
        ),
    ),
    "dewpoint": WeatherLayerSpec(
        layer_id="dewpoint",
        ...
        primary_label="Dew Point",
        unit_label="C",
        summary_template="Current 2 m dew point is {value} {unit}.",
        current_fields=("dew_point_2m", "temperature_2m", "relative_humidity_2m"),
        hourly_fields=("dew_point_2m", "temperature_2m", "relative_humidity_2m"),
        legend_ticks=(-10, 0, 10, 15, 20, 25),
        notes=(
            "2 m dew point temperature heatmap.",
            "Open-Meteo online/local only in the first multi-source wave.",
        ),
    ),
}
