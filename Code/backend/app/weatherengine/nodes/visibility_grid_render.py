...
class VisibilityGridRenderNode(BaseNode):
    """能见度网格渲染节点，基于预报数据生成能见度网格 GeoJSON。"""

    node_type: str = "weather_visibility_grid"
    _layer_id: str = "visibility"
        ...
        try:
            ...
            if latitude is None or longitude is None:
                return NodeExecutionResult(
                    ...
                    warnings=["VisibilityGridRenderNode 缺少必需输入: latitude/longitude"],
                )
            ...
            if grid_data:
                # 使用真实网格数据构建 GeoJSON
                geojson = weather_engine_service.build_visibility_geojson_from_grid(
                    grid_data,
                    layer_id,
                )
                logger.info(
                    "[VisibilityGridRenderNode] Built from grid data: layer=%s features=%d",
                ...
                geojson = weather_engine_service.build_visibility_geojson(weather, bbox)
                logger.info(
                    "[VisibilityGridRenderNode] Built from point data (fallback): layer=%s features=%d",
                    layer_id, len(geojson.get("features", [])),
                )
            ...
            try:
                ...
                artifact = ArtifactRecord(
                    ...
                    node_id=self.spec.node_id,
                    artifact_type="visibility_geojson",
                    storage_uri=artifact_ref.resource_url or "",
                    content_type="application/geo+json",
                )
            except Exception as exc:
                logger.warning("Failed to store visibility GeoJSON artifact: %s", exc)
            ...
            return NodeExecutionResult(
                node_id=self.spec.node_id,
                status=RunStatus.failed,
                warnings=[f"VisibilityGridRenderNode failed: {exc}"],
            )

    @staticmethod
    def build_spec() -> NodeSpec:
        return NodeSpec(
            node_id=VisibilityGridRenderNode.node_type,
            node_type=VisibilityGridRenderNode.node_type,
            input_ports=[
            ...
            output_ports=[
                PortSpec(name="geojson", kind=PortKind.geojson, description="能见度网格 GeoJSON FeatureCollection"),
            ],
        )
