...
class WindFieldRenderNode(BaseNode):
    """风场渲染节点，基于预报数据生成风场矢量 GeoJSON。"""

    node_type: str = "weather_wind_field"
    _layer_id: str = "wind-field"
        ...
        try:
            ...
            if latitude is None or longitude is None:
                return NodeExecutionResult(
                    ...
                    warnings=["WindFieldRenderNode 缺少必需输入: latitude/longitude"],
                )
            ...
            if grid_data:
                # 使用真实网格数据构建 GeoJSON
                geojson = weather_engine_service.build_wind_geojson_from_grid(
                    grid_data,
                    layer_id,
                )
                logger.info(
            ...
            try:
                ...
                artifact = ArtifactRecord(
                    ...
                    node_id=self.spec.node_id,
                    artifact_type="wind_geojson",
                    storage_uri=artifact_ref.resource_url or "",
                    content_type="application/geo+json",
                )
            except Exception as exc:
                logger.warning("Failed to store wind GeoJSON artifact: %s", exc)
            ...
            return NodeExecutionResult(
                node_id=self.spec.node_id,
                status=RunStatus.failed,
                warnings=[f"WindFieldRenderNode failed: {exc}"],
            )
