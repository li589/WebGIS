"""Precipitation grid render node — GeoJSON from grid or point fallback (N1 薄壳)."""
...
class PrecipitationGridRenderNode(ScalarGridRenderNode):
    """降水网格渲染节点。"""

    node_type: str = "weather_precipitation_grid"
    _layer_id: str = "precipitation"
    _metric_key: str = "precipitation"
    _unit: str = "mm"
    _min_value: float | None = 0.1
    _artifact_type: str = "precipitation_geojson"
    _result_prefix: str = "precipitation"
    _display_title: str = "Precipitation Grid GeoJSON"
    _node_label: str = "PrecipitationGridRenderNode"

    def _build_point_fallback(
        self, service: Any, weather: WeatherPointResponse, bbox: Any
    ) -> dict[str, object]:
        return service.build_precipitation_geojson(weather, bbox)
        ...
        return NodeSpec(
            ...
            output_ports=[
                PortSpec(
                    ...
                    description="降水网格 GeoJSON FeatureCollection",
                ),
            ],
        )
