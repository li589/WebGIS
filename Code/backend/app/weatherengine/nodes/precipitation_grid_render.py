        ...
        try:
            ...
            if latitude is None or longitude is None:
                return NodeExecutionResult(
                    ...
                    warnings=[
                        "PrecipitationGridRenderNode 缺少必需输入: latitude/longitude"
                    ],
                )
            ...
            if grid_data:
                ...
                geojson = weather_engine_service.build_precipitation_geojson(
                    weather, bbox
                )
                logger.info(
                    "[PrecipitationGridRenderNode] Built from point data (fallback): layer=%s features=%d",
                    layer_id,
                    len(geojson.get("features", [])),
                )
            ...
            try:
                ...
                artifact_ref = storage.create_artifact_result_ref(
                    run_id=run_id,
                    result_id=f"precipitation-geojson-{self.spec.node_id}",
                    result_kind=ResultKind.file,
                    title="Precipitation Grid GeoJSON",
                ...
                artifact = ArtifactRecord(
                    ...
                    node_id=self.spec.node_id,
                    artifact_type="precipitation_geojson",
                    storage_uri=artifact_ref.resource_url or "",
                    content_type="application/geo+json",
                )
            except Exception as exc:
                logger.warning(
                    "Failed to store precipitation GeoJSON artifact: %s", exc
                )
