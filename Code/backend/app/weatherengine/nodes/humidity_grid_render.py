...
class HumidityGridRenderNode(BaseNode):
    """湿度网格渲染节点，基于预报数据生成相对湿度网格 GeoJSON。"""
        ...
        try:
            ...
            try:
                ...
                artifact = ArtifactRecord(
                    ...
                    node_id=self.spec.node_id,
                    artifact_type="humidity_geojson",
                    storage_uri=artifact_ref.resource_url or "",
                    content_type="application/geo+json",
                )
            except Exception as exc:
                logger.warning("Failed to store humidity GeoJSON artifact: %s", exc)
            ...
            return NodeExecutionResult(
                node_id=self.spec.node_id,
                status=RunStatus.failed,
                warnings=[f"HumidityGridRenderNode failed: {exc}"],
            )
