"""Cloud-cover grid render node — GeoJSON from grid or point fallback."""
...
class CloudCoverGridRenderNode(BaseNode):
    """云量网格渲染节点。"""
        ...
        try:
            ...
            if latitude is None or longitude is None:
                return NodeExecutionResult(
                    ...
                    warnings=[
                        "CloudCoverGridRenderNode 缺少必需输入: latitude/longitude"
                    ],
                )
            ...
            if grid_data:
                ...
                # Prefer dedicated builder when present; else scalar from point
                build = getattr(
                    weather_engine_service, "build_cloud_cover_geojson", None
                )
                if callable(build):
                    geojson = build(weather, bbox)
                else:
                    base = float(getattr(weather.current, "cloud_cover", None) or 0.0)
            ...
            try:
                ...
                artifact_ref = storage.create_artifact_result_ref(
                    run_id=run_id,
                    result_id=f"cloud-cover-geojson-{self.spec.node_id}",
                ...
                artifact = ArtifactRecord(
                    ...
                    node_id=self.spec.node_id,
                    artifact_type="cloud_cover_geojson",
                    storage_uri=artifact_ref.resource_url or "",
                    content_type="application/geo+json",
                )
            except Exception as exc:
                logger.warning("Failed to store cloud-cover GeoJSON artifact: %s", exc)
            ...
            return NodeExecutionResult(
                node_id=self.spec.node_id,
                status=RunStatus.failed,
                warnings=[f"CloudCoverGridRenderNode failed: {exc}"],
            )
