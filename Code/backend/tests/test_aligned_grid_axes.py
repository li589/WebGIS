...
class TestAlignedGridAxes:
    def test_adjacent_lon_tiles_share_no_centers(self):
        res = 1.0
        left = BoundingBox(west=110.0, south=20.0, east=115.0, north=25.0, crs="EPSG:4326")
        right = BoundingBox(west=115.0, south=20.0, east=120.0, north=25.0, crs="EPSG:4326")
    ...
    def test_adjacent_lat_tiles_share_no_centers(self):
        res = 1.0
        north_tile = BoundingBox(west=110.0, south=25.0, east=115.0, north=30.0, crs="EPSG:4326")
        south_tile = BoundingBox(west=110.0, south=20.0, east=115.0, north=25.0, crs="EPSG:4326")
    ...
    def test_web_mercator_neighbors_at_high_lat(self):
        ...
        # 经度列在同 x 应对齐
        assert set(lons_a) == set(lons_b) or set(lons_a).issubset(set(lons_b)) or set(lons_b).issubset(
            set(lons_a)
        )

    def test_half_open_predicates(self):
        assert point_in_tile_half_open(110.0, 25.0, west=110, south=20, east=115, north=25)
        assert not point_in_tile_half_open(115.0, 22.0, west=110, south=20, east=115, north=25)
        assert not point_in_tile_half_open(112.0, 20.0, west=110, south=20, east=115, north=25)

    def test_centers_on_global_lattice(self):
        lons = lon_centers_half_open(-180.0, -170.0, 5.0)
        assert all(abs((lon / 5.0) - 0.5 - round(lon / 5.0 - 0.5)) < 1e-9 for lon in lons)
