...
class _FakeRunResult:
    ...
    def __init__(self, status="completed", outputs=None, errors=None, warnings=None, node_results=None):
        self.run_id = "weather-run-1"
        self.workflow_id = "test-weather-workflow"
        self.status = type("FakeStatus", (), {"value": status})() if isinstance(status, str) else status
...
class WeatherBridgeServiceTests(unittest.TestCase):
    def _make_payload(self, weather_request: WeatherWorkflowRequest | dict | None) -> WorkflowSubmitRequest:
    ...
    def test_supports_returns_false_when_disabled(self) -> None:
        with patch("app.services.weather_bridge_service.settings") as mock_settings:
            ...
            payload = self._make_payload(WeatherWorkflowRequest(workflow={"workflow_id": "x", "nodes": []}))
    ...
    def test_execute_workflow_returns_mapped_result(self) -> None:
        ...
        fake_service = type("FakeService", (), {
            ...
            "diagnose": lambda self: {"status": "ok", "node_registry": {"status": "ok", "supported_node_types": ["weather_forecast_fetch"]}},
        })()
            ...
            payload = self._make_payload(
                WeatherWorkflowRequest(
                    workflow={
                        ...
                        "nodes": [{"node_id": "n1", "node_type": "weather_summary_generate", "params": {}}],
                    },
                    workflow_id="test-weather-wf",
                )
            )
    ...
    def test_list_workflows_response_returns_node_types(self) -> None:
        ...
        fake_service = type("FakeService", (), {
            "diagnose": lambda self: {
                ...
                "node_registry": {
                    "status": "ok",
                    "supported_node_types": ["weather_forecast_fetch", "weather_point_parse", "weather_wind_field"],
                },
            },
        })()
        with patch.object(service, "_get_service", return_value=fake_service):
    ...
    def test_describe_workflow_returns_404_for_unknown(self) -> None:
        ...
        fake_service = type("FakeService", (), {
            "diagnose": lambda self: {
                "status": "ok",
                "node_registry": {"status": "ok", "supported_node_types": ["weather_forecast_fetch"]},
            },
        })()
        with patch.object(service, "_get_service", return_value=fake_service):
