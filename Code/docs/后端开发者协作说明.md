...
- 稳定分支：`main`
- 日常开发分支：`dev`
- 你主要负责目录：`Code/backend`、`Code/algorithms`、`Code/infra`
...
然后在 `Code/backend`、`Code/algorithms`、`Code/infra` 中开发。
...
你负责后端，就重点维护 `Code/backend`、`Code/algorithms`、`Code/infra`；同步时优先拉前端和共享目录，不要随手覆盖自己正在做的后端代码。
