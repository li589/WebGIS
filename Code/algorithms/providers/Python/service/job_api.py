...
from contracts.request_templates import RequestTemplateSpec, build_workflow_request_template, get_module_request_template
...
from workflow.ui_metadata import WorkflowInputPanelUiSchema, WorkflowPanelUiField, WorkflowPanelUiSection, enhance_panel_schema_with_ui_metadata
...
class JobService:
    def __init__(
        ...
        *,
        ...
        self._scheduler_adapter_factory = scheduler_adapter_factory or _constant_factory(scheduler_adapter)
        self._datasource_adapter_factory = datasource_adapter_factory or _constant_factory(datasource_adapter)
        self._logger_adapter_factory = logger_adapter_factory or _constant_factory(logger_adapter)
        self._product_sink_factory = product_sink_factory or _constant_factory(product_sink)
    ...
    def get_health_response(self) -> ServiceResponse:
        return ServiceResponse(
            ...
            body={
                ...
                "async_job_store": (
                    None if self._async_job_registry is None else type(self._async_job_registry).__name__
                ),
                "job_queue": None if self._job_queue is None else type(self._job_queue).__name__,
            },
        )

    def get_job_request_schema_response(self) -> ServiceResponse:
        return ServiceResponse(status_code=200, body=_to_jsonable(get_job_request_json_schema()))
    ...
    def submit_job(self, payload: object) -> ServiceResponse:
        ...
        if error_response is not None:
            return error_response
        scheduler_adapter, datasource_adapter, logger_adapter, product_sink = self._create_runtime_adapters()
    ...
    def _coerce_and_validate_request(self, payload: object) -> tuple[JobRequest | None, ServiceResponse | None]:
        request = None
        try:
            request = coerce_job_request(payload)
    ...
    *,
    ...
    service = JobService(
        ...
        product_sink_factory=(
            ...
            else (lambda: LocalProductSink(resolved_workspace / "products" / "manifests"))
        ),
...
def build_local_persistent_job_service(
    *,
    ...
    async_registry = FileAsyncJobRegistry(resolved_workspace / "service_state" / "submissions")
...
def build_worker(job_service: JobService) -> JobQueueWorker:
    if job_service._async_job_registry is None or job_service._job_queue is None:  # noqa: SLF001
        raise ValueError("JobService must be created with async registry and job queue.")
...
def _build_error_response(error: Exception, *, request: JobRequest | None) -> ServiceResponse:
...
def _build_workflow_summary(workflow_name: str) -> dict[str, Any]:
    ...
    preview_modes = tuple(mode for mode, _workflow in preview_variants if mode is not None)
...
def _build_workflow_description(workflow_name: str) -> dict[str, Any]:
    ...
    preview_modes = tuple(mode for mode, _workflow in preview_variants if mode is not None)
    template = _build_workflow_request_template_preview(workflow_name, preview_variants)
    panel_schema = _build_workflow_panel_schema(workflow_name, preview_variants=preview_variants)
    ui_schema = _build_workflow_ui_schema(workflow_name, preview_variants=preview_variants)
    body = {
        "name": workflow.name,
        "workflow_id": workflow.workflow_id,
        "description": workflow.description,
...
def _build_named_workflow_preview(workflow_name: str):
    return build_named_workflow(workflow_name, _build_preview_request(workflow_name=workflow_name))
...
def _build_named_workflow_previews(workflow_name: str):
    ...
    return tuple(
        (
            None if request.algorithm_params.get("mode") is None else str(request.algorithm_params["mode"]).lower(),
...
def _build_workflow_panel_schema(
    ...
    *,
    ...
    schemas = [build_workflow_input_panel_schema(workflow) for _mode, workflow in variants]
...
def _merge_request_templates(
    ...
    *,
    ...
    required_datasource_sets = [set(template.required_datasource_keys) for template in templates]
    shared_required_datasource = set.intersection(*required_datasource_sets) if required_datasource_sets else set()
    all_datasource = set().union(
        *(set(template.required_datasource_keys) | set(template.optional_datasource_keys) for template in templates)
    )
    required_algorithm_sets = [set(template.required_algorithm_keys) for template in templates]
    shared_required_algorithm = set.intersection(*required_algorithm_sets) if required_algorithm_sets else set()
    all_algorithm = set().union(
        *(set(template.required_algorithm_keys) | set(template.optional_algorithm_keys) for template in templates)
    )
    ...
    for template in templates:
        for required_key, dataset_names in template.accepted_data_access_by_required_key.items():
    ...
    notes = [note for note in {template.notes for template in templates if template.notes}]
    notes.append(
        "Preview merges workflow variants for modes: " + ", ".join(preview_modes) + ". Mode-specific inputs are listed as optional."
    )
    return RequestTemplateSpec(
        ...
        optional_datasource_keys=tuple(sorted(all_datasource - shared_required_datasource)),
        required_algorithm_keys=tuple(sorted(shared_required_algorithm)),
        optional_algorithm_keys=tuple(sorted(all_algorithm - shared_required_algorithm)),
...
def _merge_panel_schemas(
    ...
    *,
    ...
    return WorkflowInputPanelSchema(
        ...
        datasource_fields=_merge_panel_field_group([schema.datasource_fields for schema in schemas], len(schemas)),
        algorithm_param_fields=_merge_panel_field_group([schema.algorithm_param_fields for schema in schemas], len(schemas)),
        request_fields=_merge_panel_field_group([schema.request_fields for schema in schemas], len(schemas)),
        notes=tuple(
            sorted(
                {
                    *[note for schema in schemas for note in schema.notes],
                    "Preview merges workflow variants for modes: " + ", ".join(preview_modes) + ".",
...
def _merge_panel_field_group(
    ...
    for key, fields in sorted(merged.items()):
        first = fields[0]
        descriptions = tuple(dict.fromkeys(field.description for field in fields if field.description))
        result.append(
            WorkflowPanelField(
                key=first.key,
                section=first.section,
                required=len(fields) == variant_count and all(field.required for field in fields),
                value_kind=first.value_kind,
                description=" ".join(descriptions) if descriptions else None,
                consumers=tuple(sorted({item for field in fields for item in field.consumers})),
                entry_names=tuple(sorted({item for field in fields for item in field.entry_names})),
                allowed_values=tuple(sorted({item for field in fields for item in field.allowed_values})),
                source_types=tuple(sorted({item for field in fields for item in field.source_types})),
                format_hints=tuple(sorted({item for field in fields for item in field.format_hints})),
            )
        )
    return tuple(result)
