...
def _load_input_array(
    ...
    if input_path is None:
        raise ValueError(
            "statistics: 需要提供 datasource_selection.input_path 或 manifest"
        )
    ...
    def execute(
        ...
        output_dir = Path(
            output_spec_extra.get(
                "output_dir", ctx.workspace / "products" / "statistics"
            )
        )
        ...
        if mode == "global":
            ...
            raise ValueError(
                f"不支持的统计模式: {mode} (可选 global|zonal|landcover|timeseries)"
            )
        ...
        if mode == "global":
            ...
            mat_payload["std_series"] = np.array(
                [s["std"] for s in result["timeseries"]]
            )  # type: ignore[index]
