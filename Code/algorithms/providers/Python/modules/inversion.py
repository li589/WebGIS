...
class InversionDailyModule(BaseModule):
    ...
    def execute(self, inputs: dict[str, object], params: dict[str, object], ctx: NodeExecutionContext) -> dict[str, object]:
        ...
        output_dir = Path(output_spec_extra.get("output_dir", ctx.workspace / "products" / "inversion_daily"))
        output_dir.mkdir(parents=True, exist_ok=True)

        if ctx.logger_adapter is not None:
            ctx.logger_adapter.emit_stage_start("inversion_daily", f"Run {mode} inversion for {input_mat.name}")
        ...
        if ctx.logger_adapter is not None:
            ctx.logger_adapter.emit_artifact("inversion_daily", str(output_path), "inversion_mat")
            ctx.logger_adapter.emit_stage_end("inversion_daily", f"Generated {output_path.name}")
