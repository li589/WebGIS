...
class OmegaBlockModule(BaseModule):
    name = "omega_block"
    description = (
        "Native module that runs OMEGA block retrieval over a timeseries MAT bundle."
    )
    ...
    input_ports = [
        ...
        PortSpec(
            name="omega_fixed_mat", kind="scalar", data_class="path", required=False
        ),
        PortSpec(
            name="exp0_calib_mat", kind="scalar", data_class="path", required=False
        ),
    ]
    output_ports = [
    ...
    def execute(
        ...
        if mode == "omega":
            missing_keys.extend(
                key
                for key in ("omega_fixed_mat", "exp0_calib_mat")
                if key not in datasource_selection
                or datasource_selection.get(key) is None
            )
        if missing_keys:
            raise ValueError(
                f"omega_block requires datasource_selection keys for mode '{mode}': {', '.join(sorted(missing_keys))}"
            )
