...
class JobServiceTests(unittest.TestCase):
    ...
    def test_validate_job_returns_normalized_request(self) -> None:
        ...
        self.assertEqual(
            response.body["normalized_request"]["workflow_name"], "retrieval_workflow"
        )
        self.assertEqual(
            response.body["normalized_request"]["algorithm_params"]["mode"], "omega"
        )

    def test_validate_job_returns_structured_error_for_missing_omega_inputs(
        self,
    ) -> None:
        service = self._build_service(run_job_fn=lambda *args, **kwargs: None)
        ...
        self.assertEqual(
            response.body["issues"][0]["field_path"],
            "job_request.datasource_selection.omega_fixed_mat",
        )
    ...
    def test_submit_job_returns_serialized_job_result_on_success(self) -> None:
        ...
        self.assertEqual(
            response.body["job_result"]["manifest_uri"],
            "D:/workspace/manifests/run-001.json",
        )
        ...
        self.assertEqual(
            response.body["result_dto"]["artifacts"]["manifest"]["storage_backend"],
            "file",
        )
        self.assertEqual(
            response.body["result_dto"]["artifacts"]["manifest"]["object_key"],
            "D:/workspace/manifests/run-001.json",
        )
        self.assertEqual(
            response.body["result_dto"]["artifacts"]["manifest"]["preview_url"],
            "file:///D:/workspace/manifests/run-001.json",
        )
        self.assertEqual(
            response.body["result_dto"]["artifacts"]["log"]["storage_backend"], "https"
        )
        self.assertEqual(
            response.body["result_dto"]["artifacts"]["log"]["download_url"],
            "https://logs.example.com/jobs/run-001.log",
        )
        self.assertIsNone(response.body["result_dto"]["artifacts"]["metadata"]["uri"])
        self.assertEqual(
            response.body["result_dto"]["conversion_trace"]["dataset_count"], 1
        )
        self.assertEqual(
            response.body["result_dto"]["conversion_trace"]["entry_count"], 2
        )
        self.assertEqual(
            response.body["result_dto"]["conversion_trace"]["datasets"][0][
                "resource_count"
            ],
            2,
        )
        self.assertEqual(
            response.body["result_dto"]["conversion_trace"]["datasets"][0]["resources"][
                0
            ]["origin_uri"],
        ...
        self.assertEqual(
            response.body["result_dto"]["conversion_trace"]["datasets"][0]["resources"][
                1
            ]["loaded_summary"]["counts"]["dimension_count"],
            2,
        )
        self.assertEqual(
            response.body["result_dto"]["conversion_trace"]["datasets"][0]["resources"][
                1
            ]["loaded_summary"]["title"],
            "NetCDF dataset",
        )
        ...
        self.assertEqual(
            panel["datasets"][0]["resources"][1]["title"], "NetCDF dataset"
        )
        self.assertEqual(
            panel["datasets"][0]["resources"][1]["summary"]["schema"][
                "dimension_names"
            ],
            ["x", "y"],
        )
    ...
    def test_submit_job_expands_local_manifest_into_result_dto(self) -> None:
        ...
        def fake_run_job(
            ...
            manifest_path = (
                Path(workspace) / "products" / "manifests" / "run-dto-001.json"
            )
        ...
        self.assertEqual(
            result_dto["artifacts"]["metadata_uri"],
            "minio://omega-bucket/jobs/run-dto-001/metadata.json",
        )
        self.assertEqual(result_dto["artifacts"]["manifest"]["storage_backend"], "file")
        self.assertEqual(
            result_dto["artifacts"]["manifest"]["object_key"], manifest_path.as_posix()
        )
        self.assertEqual(
            result_dto["artifacts"]["metadata"]["storage_backend"], "minio"
        )
        self.assertEqual(result_dto["artifacts"]["metadata"]["bucket"], "omega-bucket")
        self.assertEqual(
            result_dto["artifacts"]["metadata"]["object_key"],
            "jobs/run-dto-001/metadata.json",
        )
        self.assertEqual(result_dto["artifacts"]["log"]["storage_backend"], "https")
        self.assertEqual(
            result_dto["artifacts"]["log"]["preview_url"],
            "https://logs.example.com/jobs/run-dto-001.log",
        )
        ...
        self.assertEqual(
            result_dto["products"][0]["object_key"], "jobs/run-dto-001/omega_block.mat"
        )
        ...
        self.assertEqual(
            result_dto["conversion_trace"]["datasets"][0]["resource_count"], 2
        )
        self.assertEqual(
        ...
        self.assertEqual(
            result_dto["conversion_trace"]["datasets"][0]["resources"][1][
                "loaded_summary"
            ]["document"]["root_tag"],
            "root",
        )
        ...
        self.assertEqual(
            result_dto["manifest_summary"]["conversion_trace_dataset_count"], 1
        )
        self.assertEqual(
            result_dto["manifest_summary"]["conversion_trace_resource_count"], 2
        )
        ...
        self.assertEqual(
            panel["datasets"][0]["resources"][1]["title"], "XML document root"
        )
        self.assertEqual(
    ...
    def test_submit_job_expands_http_and_s3_storage_fields_into_result_dto(
        self,
    ) -> None:
        def fake_run_job(
            request,
            ...
            manifest_path = (
                Path(workspace) / "products" / "manifests" / "run-dto-002.json"
            )
            manifest_path.parent.mkdir(parents=True, exist_ok=True)
            manifest_path.write_text(
                json.dumps(
        ...
        self.assertEqual(
            products[0]["preview_url"],
            "https://files.example.com/jobs/run-dto-002/omega_preview.tif",
        )
        self.assertEqual(
            products[0]["download_url"],
            "https://files.example.com/jobs/run-dto-002/omega_preview.tif",
        )
        self.assertEqual(products[1]["storage_backend"], "s3")
        self.assertEqual(products[1]["bucket"], "archive-bucket")
        self.assertEqual(
            products[1]["object_key"], "jobs/run-dto-002/omega_archive.tif"
        )
        self.assertEqual(
            products[1]["preview_url"],
            "https://signed.example.com/preview/omega_archive.tif",
        )
        self.assertEqual(
            products[1]["download_url"],
            "https://signed.example.com/download/omega_archive.tif",
        )
    ...
    def test_submit_job_expands_local_file_storage_fields_into_result_dto(self) -> None:
        ...
        def fake_run_job(
            ...
            manifest_path = (
                Path(workspace) / "products" / "manifests" / "run-dto-003.json"
            )
            manifest_path.parent.mkdir(parents=True, exist_ok=True)
            manifest_path.write_text(
                json.dumps(
    ...
    def test_submit_job_returns_execution_error_payload_when_run_job_fails(
        self,
    ) -> None:
        def fake_run_job(
            request,
    ...
    def test_list_modules_and_describe_module_return_structured_metadata(self) -> None:
        ...
        self.assertTrue(
            any(
                item["name"] == "omega_block"
                for item in modules_response.body["modules"]
            )
        )
        ...
        self.assertTrue(
            any(
                port["name"] == "input_mat"
                for port in describe_response.body["input_ports"]
            )
        )
        self.assertEqual(
            describe_response.body["request_template"]["entry_name"], "omega_block"
        )
    ...
    def test_list_workflows_and_describe_workflow_return_panel_metadata(self) -> None:
        ...
        panel_response = service.get_workflow_panel_schema_response(
            "retrieval_workflow"
        )
        ...
        self.assertEqual(
            describe_response.body["definition"]["workflow_id"], "retrieval_workflow"
        )
        ...
        self.assertTrue(
            any(
                field["key"] == "algorithm_params"
                for field in panel_response.body["request_fields"]
            )
        )
        self.assertTrue(
            any(
                field["key"] == "mode"
                for field in panel_response.body["algorithm_param_fields"]
            )
        )
        self.assertTrue(
            any(
                field["key"] == "omega_fixed_mat"
                for field in panel_response.body["datasource_fields"]
            )
        )
        self.assertTrue(
            any(
                field["key"] == "exp0_calib_mat"
                for field in panel_response.body["datasource_fields"]
            )
        )
        self.assertTrue(
            any(
                "Preview merges workflow variants" in note
                for note in panel_response.body["notes"]
            )
        )
        ...
        self.assertTrue(
            any(
                section["key"] == "datasource_selection"
                for section in ui_response.body["sections"]
            )
        )
        datasource_section = next(
            section
            for section in ui_response.body["sections"]
            if section["key"] == "datasource_selection"
        )
        self.assertTrue(
            any(
                field["key"] == "omega_fixed_mat"
                for field in datasource_section["fields"]
            )
        )
    ...
    def test_submit_job_async_returns_submission_and_status_can_be_queried(
        self,
    ) -> None:
        def fake_run_job(
            request,
            ...
            scheduler_adapter.update_status(
                request.job_id, "run-async-001", "running", {"stage": "dispatch"}
            )
            result = JobResult(
                job_id=request.job_id,
