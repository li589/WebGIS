...
def coerce_workflow_definition(value: object) -> WorkflowDefinition:
    ...
    if isinstance(value, str):
        try:
            ...
            raise WorkflowDefinitionDecodeError(
                f"workflow_definition JSON decode failed: {exc.msg}"
            ) from exc
        if not isinstance(payload, Mapping):
            raise WorkflowDefinitionDecodeError(
                "workflow_definition JSON payload must decode to an object"
            )
...
def workflow_definition_from_mapping(
    ...
    version = (
        _optional_string(payload, "version", "workflow_definition.version") or "1.0"
    )
    name = _optional_string(payload, "name", "workflow_definition.name")
    description = _optional_string(
        payload, "description", "workflow_definition.description"
    )
    inputs_payload = _optional_mapping(
        payload, "inputs", "workflow_definition.inputs", default={}
    )
    nodes_payload = _require_list(payload, "nodes", "workflow_definition")
    edges_payload = _optional_list(
        payload, "edges", "workflow_definition.edges", default=[]
    )
    outputs_payload = _require_list(payload, "outputs", "workflow_definition")
    defaults = _optional_mapping(
        payload, "defaults", "workflow_definition.defaults", default={}
    )
    ...
    inputs = {
        input_name: _parse_input_source_spec(
            input_value, path=f"workflow_definition.inputs.{input_name}"
        )
        for input_name, input_value in inputs_payload.items()
    }
    nodes = [
        _parse_workflow_node_spec(
            node_payload, path=f"workflow_definition.nodes[{index}]"
        )
    ...
    outputs = [
        _parse_workflow_output_spec(
            output_payload, path=f"workflow_definition.outputs[{index}]"
        )
...
def _parse_input_source_spec(value: object, *, path: str) -> InputSourceSpec:
    ...
    field_map_payload = _optional_mapping(
        payload, "field_map", f"{path}.field_map", default={}
    )
    return InputSourceSpec(
        ...
        field_map={
            alias_name: _as_string_list(
                alias_value, path=f"{path}.field_map.{alias_name}"
            )
            for alias_name, alias_value in field_map_payload.items()
        },
        selector=dict(
            _optional_mapping(payload, "selector", f"{path}.selector", default={})
        ),
        options=dict(
            _optional_mapping(payload, "options", f"{path}.options", default={})
        ),
    )
...
def _parse_workflow_node_spec(value: object, *, path: str) -> WorkflowNodeSpec:
    ...
    return WorkflowNodeSpec(
        ...
        input_bindings=_as_string_mapping(
            _optional_mapping(
                payload, "input_bindings", f"{path}.input_bindings", default={}
            ),
            path=f"{path}.input_bindings",
        ),
        params=dict(_optional_mapping(payload, "params", f"{path}.params", default={})),
        cache_policy=_optional_mapping_or_none(
            payload, "cache_policy", f"{path}.cache_policy"
        ),
        retry_policy=_optional_mapping_or_none(
            payload, "retry_policy", f"{path}.retry_policy"
        ),
...
def _parse_workflow_edge(value: object, *, path: str) -> WorkflowEdge:
    payload = _as_mapping(value, path)
    _reject_unknown_fields(
        payload, {"from_node", "from_port", "to_node", "to_port"}, path=path
    )
    return WorkflowEdge(
...
def _require_string(payload: Mapping[str, object], key: str, path: str) -> str:
    ...
    if not isinstance(value, str) or not value.strip():
        raise WorkflowDefinitionDecodeError(
            f"Field must be a non-empty string: {path}.{key}"
        )
    return value
...
def _optional_list(
    payload: Mapping[str, object], key: str, path: str, *, default: list[object]
) -> list[object]:
    if key not in payload or payload[key] is None:
        return list(default)
...
def _as_string_list(value: object, *, path: str) -> list[str]:
    ...
    for index, item in enumerate(items):
        if not isinstance(item, str):
            raise WorkflowDefinitionDecodeError(
                f"Array item must be a string: {path}[{index}]"
            )
        result.append(item)
    return result
...
def _as_string_mapping(value: Mapping[str, object], *, path: str) -> dict[str, str]:
    ...
    for key, item in value.items():
        if not isinstance(item, str):
            raise WorkflowDefinitionDecodeError(
                f"Mapping value must be a string binding: {path}.{key}"
            )
        result[key] = item
    return result
