...
def _resolve_inversion_input_mat(datasource_selection: dict[str, object]) -> Path:
    prepared_path = resolve_prepared_local_path(datasource_selection, ("daily_bundle_mat", "input_mat"))
...
class InversionDailyPipeline(BasePipeline):
    ...
    def plan(self, request: JobRequest, ctx: RuntimeContext) -> PipelinePlan:
        ...
        return PipelinePlan(
            ...
            required_variables=["TBv", "TBh", "Ts", "Tau_ini", "CF", "Albedo", "porosity", "Theta"],
    ...
    def execute(self, request: JobRequest, ctx: RuntimeContext) -> ProductManifest:
        ...
        output_dir = Path(request.output_spec.extra.get("output_dir", ctx.workspace / "products" / "inversion_daily"))
        output_dir.mkdir(parents=True, exist_ok=True)

        if self.logger_adapter is not None:
            self.logger_adapter.emit_stage_start("inversion_daily", f"Run {mode} inversion for {input_mat.name}")
        ...
        if mode == "dh":
            ...
            products = [ProductRef(name=f"{input_mat.stem}_dh", type="dh_mat", uri=str(output_path), variable="DH")]
            main_layers = ["DH"]
        else:
            ...
            products = [
                ProductRef(name=f"{input_mat.stem}_sm", type="sm_mat", uri=str(output_path), variable="SM"),
                ProductRef(name=f"{input_mat.stem}_vod", type="vod_mat", uri=str(output_path), variable="VOD"),
            ]
            main_layers = ["SM", "VOD"]

        if self.logger_adapter is not None:
            self.logger_adapter.emit_artifact("inversion_daily", str(output_path), "inversion_mat")
            self.logger_adapter.emit_stage_end("inversion_daily", f"Generated {output_path.name}")
