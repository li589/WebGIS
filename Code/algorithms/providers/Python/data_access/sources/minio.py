...
class MinioSource:
    ...
    def materialize(
        ...
        *,
        ...
        """
        destination_root = Path(target_dir) if target_dir is not None else Path.cwd() / ".data" / "minio_cache"
        ...
        if not bucket or not object_key:
            raise ValueError(f"MinIO materialize requires bucket and object_key for {resource.uri}")

        safe_name = object_key.replace("/", "_")
        local_path = destination_root / safe_name

        endpoint = (
            os.getenv("BACKEND_MINIO_ENDPOINT")
            or os.getenv("MINIO_ENDPOINT")
            or ""
        ).strip()
        access_key = (
            os.getenv("BACKEND_MINIO_ACCESS_KEY")
            or os.getenv("MINIO_ACCESS_KEY")
            or ""
        ).strip()
        secret_key = (
            os.getenv("BACKEND_MINIO_SECRET_KEY")
            or os.getenv("MINIO_SECRET_KEY")
            or ""
        ).strip()
        secure = (
            os.getenv("BACKEND_MINIO_SECURE")
        ...
        try:
            ...
            raise ValueError("minio package is required to materialize s3/minio URIs") from exc
        ...
        try:
            ...
            raise ValueError(f"MinIO materialize failed for {resource.uri}: {exc}") from exc
