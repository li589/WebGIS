...
class Resolver:
    def resolve(
        ...
        selected = tuple(resource for resource in resources if self._matches_request(request, resource))
        if not selected:
            return ()
        if request.preferred_format is None:
            return selected
        preferred_format = normalize_format(request.preferred_format)
        preferred = tuple(resource for resource in selected if resource.format == preferred_format)
        fallback = tuple(resource for resource in selected if resource.format != preferred_format)
    ...
    def _matches_request(request: DataRequestV2, resource: ResourceRef) -> bool:
        ...
        if request.accepted_formats:
            ...
            if resource.format not in accepted:
                return False
        if request.source_hints and resource.source_kind not in set(request.source_hints):
            return False
        return True
