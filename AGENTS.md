...
面向**人**（接手者 / 新成员 / 运维）的部署与再开发手册：**`Docs/04-执行部署/部署与再开发指南.md`**（首次部署 SOP、三态启停、生产必改清单、验收、故障速查、再开发环境、避坑清单）。本文是"在哪 / 跑什么"的导航，那份是"怎么做"的流程，两者不重复。
...
| 账号口令 / 凭据轮换 | `Code/backend/scripts/rotate_user_passwords.py`、`Code/backend/.env`（`BACKEND_ADMIN_PASSWORD` **仅首次播种**）、`Test/debug/_auth.py`、小程序 `services/config.local.js`（gitignore） | `cd Code/backend && python scripts/rotate_user_passwords.py list --probe cgda-dev-admin`（**exit 3 = 仍有账号接受该口令**）。注意：改 `.env` 口令对**已存在账号无效**，必须走脚本轮换（`rotate --all --probe <旧口令>`），并复核为 exit 0。改口令只会改 `password_hash`，**不会吊销既有会话**——脚本默认会吊销（`--keep-sessions` 可关），漏了可用 `revoke-sessions --all` 补救 |
| 鉴权加固（P2） | `app/services/login_lockout.py`（账号失败锁定）、`app/services/passwords.py` + `user_repository.py`（口令策略下沉存储层）、`session_service.purge_expired_sessions`（SQLite 过期会话清理）、`credential_resolver.is_loopback_host`（环回判定）、`theme_repository.assert_svg_safe`（主题 SVG logo 脚本拦截） | `Env/Python312/python.exe -m pytest Test/backend/test_auth_p2_hardening.py Test/backend/test_auth.py Test/backend/test_auth_boundary_hardening.py Test/backend/test_themes.py -q`。改口令策略时注意同步更新测试夹具口令（`test_themes.py` / `test_auth.py` 曾因弱口令与"含用户名"被策略拦下） |
