...
class WorkflowPanelSchemaTests(unittest.TestCase):
    def test_build_workflow_input_panel_schema_exposes_required_inputs_and_algorithm_fields(
        self,
    ) -> None:
        definition = WorkflowDefinition(
            ...
            outputs=[
                WorkflowOutputSpec(
                    name="final_manifest", source="node:omega_node.manifest"
                )
            ],
        )
    ...
    def test_build_workflow_input_panel_schema_exposes_template_derived_optional_fields(
        self,
    ) -> None:
        definition = WorkflowDefinition(
        ...
        self.assertEqual(
            datasource_fields["lin_pix_mat"].entry_names, ("daily_bundle",)
        )
    ...
    def test_build_workflow_input_panel_ui_schema_exposes_labels_controls_and_examples(
        self,
    ) -> None:
        definition = WorkflowDefinition(
            ...
            outputs=[
                WorkflowOutputSpec(
                    name="final_manifest", source="node:fy_node.manifest"
                )
            ],
        )
        ...
        datasource_fields = {
            field.key: field for field in sections["datasource_selection"].fields
        }
        algorithm_fields = {
            field.key: field for field in sections["algorithm_params"].fields
        }
        ...
        self.assertEqual(
            datasource_fields["input_dir"].control_type, "directory_picker"
        )
        ...
        self.assertEqual(
            request_fields["datasource_selection"].control_type, "json_editor"
        )

    def test_enhance_panel_schema_with_ui_metadata_maps_request_specific_controls(
        self,
    ) -> None:
        ...
        request_section = {section.key: section for section in ui_schema.sections}[
            "request"
        ]
