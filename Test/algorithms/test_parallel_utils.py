...
class TestAutoProcessCount(unittest.TestCase):
    ...
    def test_env_cap_invalid_value_ignored(self) -> None:
        # 非数字环境变量应被忽略，回退到 cpu_based（sibling=1 关闭共享）
        with mock.patch.dict(
            os.environ,
            {
                "CGDA_MAX_PARALLEL_WORKERS": "not-a-number",
                "CGDA_ESTIMATED_SIBLING_TASKS": "1",
            },
        ):
            with mock.patch(
                "algorithms._parallel._get_psutil_safely", return_value=None
    ...
    def test_psutil_based_memory_cap(self) -> None:
        ...
        sibling_cap = 8 // 1 = 8
        final = min(6, 10, inf, 8, chunk_cap) = 6
        """
        ...
        with mock.patch(
            ...
            with mock.patch.dict(os.environ, {"CGDA_ESTIMATED_SIBLING_TASKS": "1"}):
    ...
    def test_psutil_memory_is_binding_constraint(self) -> None:
        ...
        final = min(14, 1, inf, sibling, chunk_cap) = 1
        """
        fake_psutil = mock.MagicMock()
    ...
    def test_psutil_virtual_memory_failure_falls_back_to_cpu(self) -> None:
        ...
        with mock.patch(
            ...
            with mock.patch.dict(os.environ, {"CGDA_ESTIMATED_SIBLING_TASKS": "1"}):
                os.environ.pop("CGDA_MAX_PARALLEL_WORKERS", None)
                result = auto_process_count(chunk_count=100, max_workers=None)
        # cpu_based=6, mem_based=cpu_based=6, env_cap=inf, sibling=8 → 6
        self.assertEqual(result, 6)
    ...
    def test_sibling_cap_shares_cores(self) -> None:
        """Default sibling estimate reserves headroom across concurrent solo workers."""
        with mock.patch("algorithms._parallel._get_psutil_safely", return_value=None):
            with mock.patch("os.cpu_count", return_value=16):
                with mock.patch.dict(
                    ...
                    os.environ.pop("CGDA_MAX_PARALLEL_WORKERS", None)
                    result = auto_process_count(chunk_count=100, max_workers=None)
        # cpu_based=14, sibling_cap=16//4=4 → 4
