...
class WorkflowLifecycleFailureTests(unittest.TestCase):
    """Failure handling: classification routing, timeout, diagnostics, retry fallback."""
    ...
    def test_handle_workflow_failure_retryable(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                self.assertEqual(run.status, ExecutionStatus.retry_pending)
            finally:
                repository.close()

    def test_handle_workflow_failure_non_retryable(self) -> None:
    ...
    def test_handle_workflow_failure_max_attempts_exceeded(self) -> None:
    ...
    def test_handle_workflow_timeout(self) -> None:
    ...
    def test_finalize_workflow_failure_validation_error_message(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                self.assertIn("invalid layer parameter", run.message)
            finally:
                repository.close()

    def test_finalize_workflow_failure_skips_protected_terminal(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                self.assertEqual(run.message, "already succeeded")
            finally:
                repository.close()
    ...
    def test_finalize_workflow_failure_diagnostics(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                self.assertIn("error_type=RuntimeError", diagnostics_text)
                self.assertIn("error_message=boom", diagnostics_text)
                self.assertIn("failure_category=terminal_failure", diagnostics_text)
                self.assertIn("retryable=False", diagnostics_text)
            finally:
                repository.close()
