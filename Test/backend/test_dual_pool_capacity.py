...
class DualPoolRepositoryTests(unittest.TestCase):
    ...
    def test_count_active_runs_includes_retry_pending(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            repository = SQLiteWorkflowRepository(state_dir=Path(tmpdir))
            try:
                repository.save_run(
                ...
                self.assertEqual(
                    repository.count_active_runs(run_class=RUN_CLASS_BUSINESS), 1
                )
            finally:
                repository.close()

    def test_save_run_under_capacity_rejects_when_full(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            repository = SQLiteWorkflowRepository(state_dir=Path(tmpdir))
            try:
                repository.save_run(
                ...
                self.assertIn("capacity reached", str(ctx.exception).lower())
                self.assertEqual(
                    repository.count_active_runs(run_class=RUN_CLASS_BUSINESS), 1
                )
            finally:
    ...
    def test_save_run_under_capacity_accepts_within_limit(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            repository = SQLiteWorkflowRepository(state_dir=Path(tmpdir))
            try:
                repository.save_run_under_capacity(
    ...
    def test_save_run_under_capacity_is_atomic_under_contention(self) -> None:
        import threading
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                self.assertEqual(results.count("reject"), 5)
                self.assertEqual(
                    repository.count_active_runs(run_class=RUN_CLASS_BUSINESS), 3
                )
            finally:
                repository.close()
