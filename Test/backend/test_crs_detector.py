...
class TestDetectFromBounds:
    ...
    def test_projected_bounds_utm(self):
        """投影坐标系 bounds（UTM 数值）：推断 UTM 带，需确认。"""
        result = crs_detector.detect_from_bounds(
            (447950.0, 4419460.0, 448000.0, 4419500.0)
        )
        assert result.source_crs.startswith("EPSG:326")
        assert result.source_crs == "EPSG:32650"
...
# ── Web Mercator / XY swap ─────────────────────────────────────────────


class TestWebMercatorAndXySwap:
    def test_web_mercator_large_window(self):
        # max_abs > 5e6 且跨度 > 1e6，避免误伤 UTM 小窗
        result = crs_detector.detect_from_bounds(
            (-10_000_000.0, -8_000_000.0, 10_000_000.0, 8_000_000.0)
        )
        assert result.source_crs == "EPSG:3857"
        assert result.method == "bounds_heuristic"
        assert result.needs_user_confirm is True

    def test_detect_xy_swap_true(self):
        swapped, note = crs_detector.detect_xy_swap(
            (39.9, 116.4, 40.0, 116.5), source_crs="EPSG:4326"
        )
        assert swapped is True
        assert "颠倒" in note

    def test_detect_xy_swap_false(self):
        swapped, note = crs_detector.detect_xy_swap(
            (116.0, 39.0, 117.0, 40.0), source_crs="EPSG:4326"
        )
        assert swapped is False
        assert "正常" in note

    def test_detect_xy_swap_skips_projected(self):
        swapped, note = crs_detector.detect_xy_swap(
            (447000.0, 4419000.0, 448000.0, 4420000.0),
