...
def client(monkeypatch: pytest.MonkeyPatch) -> TestClient:
    ...
    conftest 设 ``BACKEND_ENV=test``，而 ``deps.require_write_access`` 的免鉴权
    逃逸口仅认 ``environment=="development"``，故测试环境必须显式提供 API key
    并在请求头携带，否则写端点（含 ``/import/*``）返回 503。

    ``settings`` 是 frozen dataclass，无法 setattr；直接 patch
    ``get_backend_auth_key``（``require_write_access`` 内部 ``from ... import`` 逐次取值）。
    """
    monkeypatch.setattr(
