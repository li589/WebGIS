...
def test_builds_view_model_from_run_status() -> None:
    ...
    try:
        ...
        assert view is not None, 'view is not None'
        assert view.run_id == "run-1", 'view.run_id == "run-1"'
        assert view.title == "analysis_workflow", 'view.title == "analysis_workflow"'
        assert view.subtitle == "wind-field", 'view.subtitle == "wind-field"'
        assert view.status_text == "done", 'view.status_text == "done"'
        assert view.progress_text == "100%", 'view.progress_text == "100%"'
        assert view.can_show_link, 'view.can_show_link is truthy'
        assert view.result_url == "https://example.test/artifacts/ref-1", 'view.result_url == "https://example.test/artifacts/ref-1"'
        assert len(view.metric_rows) >= 4, 'len(view.metric_rows) >= 4'
        assert view.metric_rows[0].label == "Entry", 'view.metric_rows[0].label == "Entry"'
    finally:
