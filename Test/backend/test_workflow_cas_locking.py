...
def test_save_run_cas_success() -> None:
...
def test_save_run_cas_terminal_conflict_raises() -> None:
...
def test_save_run_cas_non_terminal_conflict_retries() -> None:
...
def test_save_run_cas_retries_exhausted_raises() -> None:
...
def test_save_run_cas_run_not_found_raises() -> None:
    """CAS update on a non-existent run_id raises ConcurrentModificationError."""
    with tempfile.TemporaryDirectory() as tmpdir:
        repository = SQLiteWorkflowRepository(state_dir=Path(tmpdir))
        try:
            with pytest.raises(ConcurrentModificationError) as ctx:
                repository.save_run_cas(
                    _status("nonexistent-run", status=ExecutionStatus.succeeded),
                    expected_status=ExecutionStatus.running,
                )
            assert "not found" in str(ctx.value).lower(), '"not found" in str(ctx.exception).lower()'
        finally:
            repository.close()


def test_save_run_status_cas_delegates() -> None:
...
def test_lifecycle_finalize_success_cas_conflict_skips() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            assert run is not None, 'run is not None'
            assert run.status == ExecutionStatus.running, 'run.status == ExecutionStatus.running'
        finally:
            repository.close()


def test_lifecycle_finalize_failure_cas_conflict_skips() -> None:
...
def test_lifecycle_cancel_cas_conflict_raises_valueerror() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            with patch.object(
                ...
                assert "concurrently" in str(ctx.value).lower(), '"concurrently" in str(ctx.exception).lower()'
...
def test_concurrent_cas_only_one_wins() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            assert results.count("success") == 1, 'results.count("success") == 1'
            assert results.count("conflict") == 1, 'results.count("conflict") == 1'

            final = repository.get_run("run-race")
            assert final is not None, 'final is not None'
            assert final.status in (ExecutionStatus.succeeded, ExecutionStatus.failed), 'final.status in (ExecutionStatus.succeeded, ExecutionStatus.failed)'
        finally:
            repository.close()
