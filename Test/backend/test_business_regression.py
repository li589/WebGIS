...
def test_workflow_engine_dag() -> None:
    ...
    check(
        "DAG 正常执行 — status=completed",
        result.status == RunStatus.completed,
        f"status={result.status}",
    )
    check(
        "DAG 正常执行 — add.result=7",
        result.outputs.get("add.result") == 7,
        f"outputs={result.outputs}",
    )
    check(
        "DAG 正常执行 — mul.final=70",
        result.outputs.get("mul.final") == 70,
        f"outputs={result.outputs}",
    )
    ...
    wf2 = WorkflowDefinition(
        ...
        nodes=[
            NodeSpec(
                node_id="add", node_type="test_add", params={"a": 3, "step": 4}
            ),  # params
        ],
    )
    ...
    check(
        "输入优先级 — 全局输入覆盖 params",
        result2.outputs.get("add.result") == 300,
        f"result={result2.outputs.get('add.result')}",
    )
    ...
    check(
        "节点失败 — status=failed",
        result_fail.status == RunStatus.failed,
        f"status={result_fail.status}",
    )
    check(
        "节点失败 — errors 非空",
        len(result_fail.errors) > 0,
        f"errors={result_fail.errors}",
    )
...
def test_weather_workflow_e2e() -> None:
    ...
    check(
        "天气工作流 — status=completed",
        result.status == RunStatus.completed,
        f"status={result.status}",
    )
    check(
        "天气工作流 — summary 输出存在",
        "summary.summary" in result.outputs,
        f"outputs keys={list(result.outputs.keys())}",
    )
    check(
        "天气工作流 — summary 内容正确",
        result.outputs.get("summary.summary") == "风速 5.2 m/s",
        f"summary={result.outputs.get('summary.summary')}",
    )
    check(
        "天气工作流 — diagnostics 输出存在",
        "summary.diagnostics" in result.outputs,
        f"outputs keys={list(result.outputs.keys())}",
    )
    ...
    check(
        "诊断 — supported_node_types 正确",
        "weather_forecast_fetch" in diag["node_registry"]["supported_node_types"],
    )
...
def test_weather_point_upstream_consumption() -> None:
    ...
    for cls in [
        ...
        check(
            f"{cls.node_type} — 无 forecast 端口",
            "forecast" not in port_names,
            f"ports={port_names}",
        )
        check(
            f"{cls.node_type} — 有 weather_point 端口",
            "weather_point" in port_names,
            f"ports={port_names}",
        )
        # weather_point 应该是 required=False（支持独立运行）
        wp_port = next((p for p in spec.input_ports if p.name == "weather_point"), None)
        check(
            f"{cls.node_type} — weather_point required=False",
            wp_port and not wp_port.required,
            f"required={wp_port.required if wp_port else 'N/A'}",
        )
    ...
    check(
        "PointParseNode — 有 forecast 端口", "forecast" in pp_ports, f"ports={pp_ports}"
    )
    check(
        "PointParseNode — forecast required=False",
        next((p for p in pp_spec.input_ports if p.name == "forecast"), None) is not None
        and not next(p for p in pp_spec.input_ports if p.name == "forecast").required,
        f"ports={pp_ports}",
    )
    ...
    with (
        ...
        mock_svc._build_wind_geojson = MagicMock(
            return_value={"type": "FeatureCollection", "features": []}
        )
        mock_svc.build_wind_geojson = MagicMock(
            return_value={"type": "FeatureCollection", "features": []}
        )
        ...
        check(
            "weather_point 消费 — get_point_weather 未被调用",
            not mock_svc.get_point_weather.called,
            "get_point_weather should NOT be called when weather_point is provided",
        )
        check(
            "weather_point 消费 — WeatherPointResponse.model_validate 被调用",
            mock_wpr.model_validate.called,
            "should reconstruct WeatherPointResponse from upstream weather_point dict",
        )
        check(
            "weather_point 消费 — build_wind_geojson 被调用",
            mock_svc.build_wind_geojson.called,
            "should call _build_wind_geojson with reconstructed WeatherPointResponse",
        )
        check(
            "weather_point 消费 — status=completed",
...
def test_workflow_dispatch() -> None:
    ...
    check(
        "GEE 调度 — queue=gee-standard",
        resolve_workflow_queue(gee_payload) == settings.workflow_queue_gee_standard,
    )
    ...
    weather_payload = WorkflowSubmitRequest(
        ...
        weather_request=WeatherWorkflowRequest(
            workflow={"workflow_id": "test", "nodes": []}
        ),
    )
    check(
        "Weather 调度 — channel=weather",
        resolve_workflow_channel(weather_payload) == "weather",
    )
    check(
        "Weather 调度 — queue=weather-standard",
        resolve_workflow_queue(weather_payload)
        == settings.workflow_queue_weather_standard,
    )
    ...
    check(
        "Weather 调度 — realtime queue=weather-realtime",
        resolve_workflow_queue(weather_realtime)
        == settings.workflow_queue_weather_realtime,
    )
    ...
    check(
        "Weather 调度 — batch queue=weather-batch",
...
def test_routes() -> None:
    ...
    check(
        "GEE 路由 — 存在", len(gee_routes) >= 5, f"gee routes count={len(gee_routes)}"
    )
    ...
    check(
        "路由顺序 — diagnostics 在 {workflow_name} 之前",
        diag_idx < dyn_idx,
        f"diag_idx={diag_idx}, dyn_idx={dyn_idx}",
    )
...
def test_config_consistency() -> None:
    ...
    # 6b. Weather 配置
    check(
        "Weather — weather_workflow_enabled 存在",
        hasattr(settings, "weather_workflow_enabled"),
    )
    check(
        "Weather — weather_workflow_enabled=true",
        settings.weather_workflow_enabled is True,
    )
    check(
        "Weather — 4 个队列",
        all(
    ...
    check(
        "runtime_status_service — gee_bridge_service 注册",
        "gee_bridge_service" in service_names,
        f"services={service_names}",
    )
    check(
        "runtime_status_service — weather_bridge_service 注册",
        "weather_bridge_service" in service_names,
        f"services={service_names}",
    )

    # 6f. 服务详情包含 weather/gee 队列配置
    weather_svc = next(
        (s for s in status.services if s.service_name == "weather_bridge_service"), None
    )
    gee_svc = next(
        (s for s in status.services if s.service_name == "gee_bridge_service"), None
    )
    if weather_svc and weather_svc.details:
        queues_str = json.dumps(weather_svc.details, ensure_ascii=False, default=str)
        check(
            "weather 服务详情 — 队列配置存在",
            "weather-realtime" in queues_str or "queues" in weather_svc.details,
            f"details keys={list(weather_svc.details.keys())}",
        )
    else:
    ...
    if gee_svc and gee_svc.details:
        ...
        check(
            "gee 服务详情 — 队列配置存在",
            "gee-realtime" in queues_str or "queues" in gee_svc.details,
            f"details keys={list(gee_svc.details.keys())}",
        )
    else:
...
def test_weather_bridge_e2e() -> None:
    ...
    check(
        "bridge execute — message 包含天气工作流",
        "天气工作流" in result.message,
        f"message={result.message}",
    )
    check(
        "bridge execute — result_refs 非空",
        len(result.result_refs) == 1,
        f"count={len(result.result_refs)}",
    )
    check(
        "bridge execute — result_kind=json",
        result.result_refs[0].result_kind.value == "json"
        if hasattr(result.result_refs[0].result_kind, "value")
        else str(result.result_refs[0].result_kind) == "json",
    )
    check(
        "bridge execute — result_dto workflow_entry_name",
        result.result_dto.get("workflow_entry_name") == "bridge-e2e-test",
        f"entry={result.result_dto.get('workflow_entry_name')}",
    )
    check(
        "bridge execute — events 非空",
        len(result.events) == 2,
        f"events={len(result.events)}",
    )
    check(
        "bridge execute — diagnostics 非空",
        len(result.diagnostics) > 0,
        f"diagnostics count={len(result.diagnostics)}",
    )
