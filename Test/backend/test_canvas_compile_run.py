...
def test_compile_and_run_data_source_to_config_read() -> None:
    ...
    assert "config_read" in list_modules(), '"config_read" in list_modules()'
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert "manifest" in result.outputs, '"manifest" in result.outputs'
        assert result.execution_order == ["n1", "n2"], 'result.execution_order == ["n1", "n2"]'
        # Ensure config was actually read
        n2_out = result.node_outputs["n2"]
        assert n2_out.get("config") == {"k": 42}, 'n2_out.get("config") == {"k": 42}'
