...
import { createRunLayersSlice, type RunLayersSliceDeps } from '@/stores/layers/run-layers'
import { extractOverlayImportsFromResultRefs } from '@/stores/layers/result-adapter'
import { isOverlayDismissed, isRunDismissed } from '@/stores/layers/workspace-persist'
...
type MaterializeResult = Awaited<ReturnType<typeof materializeWorkflowMapLayers>>
...
function makeGroup(overrides: Partial<ActiveRunLayerGroup> = {}): ActiveRunLayerGroup {
  idSeq += 1
  return {
    groupId: `grp-${idSeq}`,
    runId: '',
...
function setup(): SetupResult {
  ...
  const deps: RunLayersSliceDeps = {
    ...
    addLayer: (catalogId: string, _isAdminBoundary?: boolean, jobLayer?: JobLayerItem) => {
    ...
    removeLayer: (instanceId: string) => {
      ...
      if (layer.runGroupId && sliceRef) {
        const g = sliceRef.runLayerGroups.value.find((x) => x.groupId === layer.runGroupId)
        if (g) {
          g.memberInstanceIds = g.memberInstanceIds.filter((id) => id !== instanceId)
          if (!g.memberInstanceIds.length) {
            sliceRef.runLayerGroups.value = sliceRef.runLayerGroups.value.filter(
    ...
    assignLayerAccent: () => ({ accentColor: '#3b82f6', accentGlow: 'rgba(59,130,246,.4)', chipTone: 'blue' }),
...
describe('emitWorkflowProgressTimeSeek', () => {
  it('非 running 状态不发射', () => {
    const { slice } = setup()
    slice.setJobLayers([makeJob({ jobId: 'run-1', catalogId: 'cat-1', status: 'succeeded' })])
  ...
  it('同 jobId+timeKey 去重（不重复发射）', () => {
    ...
    slice.emitWorkflowProgressTimeSeek(job, 'running', { phase: 'block_refresh', timeKey: '20240501' })
  ...
  it('range 型 timeKey 透传为 sliceLabel', () => {
    ...
    expect(slice.workflowProgressTimeSeek.value?.sliceLabel).toBe('20240501_20240508')
  })
...
describe('upsertJobLayer', () => {
  ...
  it('已存在的 job 原位替换', () => {
    ...
    slice.upsertJobLayer('cat-a', makeJob({ jobId: 'a', status: 'succeeded', progress: 100 }))
  ...
  it('cancelled 终态会 forgetTrackedWorkflowRun', () => {
    const { slice, calls } = setup()
    slice.upsertJobLayer('cat-a', makeJob({ jobId: 'run-c', status: 'cancelled' }))
  ...
  it('failed 真实 run 触发组清理：占位移除、产物解锁并标（部分）', () => {
    ...
    slice.upsertJobLayer('cat-src', makeJob({ jobId: 'run-f', status: 'failed' }))
    expect(activeLayers.map((l) => l.instanceId)).not.toContain(members[1]!.instanceId)
  ...
  it('local-submit 失败按 catalog 清理占位组', () => {
    ...
    slice.upsertJobLayer('cat-src', makeJob({ jobId: 'local-1', status: 'failed', message: '提交失败' }))
    expect(group.status).toBe('failed')
    expect(group.dissolvable).toBe(true)
...
describe('buildWorkflowPayloadForCatalog', () => {
  it('默认字段：layer_id 回退 catalogId，参数来自地图上下文', () => {
    ...
    expect(payload['parameters']).toMatchObject({ hour: 12, latitude: 39.9, longitude: 116.4 })
    expect(payload['map_context']).toMatchObject({ active_layer_id: 'cat-x', map_mode: '2d' })
...
describe('formatProgressiveSyncMessage', () => {
  it('四种分支文案', () => {
    const { slice } = setup()
    expect(slice.formatProgressiveSyncMessage(0, true)).toBe(WORKFLOW_COPY.progressiveSyncFailed)
...
describe('applyProgressiveSyncToJob', () => {
  ...
  it('job 不存在时安全无操作', () => {
    const { slice } = setup()
    expect(() => slice.applyProgressiveSyncToJob('cat-1', 'run-none', 1, false)).not.toThrow()
  })
})
...
describe('syncProgressiveBlockOverlays', () => {
  it('节流：8 秒内第二次调用直接跳过', async () => {
    ...
    const { slice } = setup()
    slice.setJobLayers([makeJob({ jobId: 'run-1', catalogId: 'cat-1' })])
    mockMaterialize([{ overlay_layer_id: 'ov-1', title: 'SM', product_tag: 'SM' }])
...
describe('attachAlgorithmProductOverlays', () => {
  it('run 已 dismiss 返回 0 且不物化', async () => {
    const { slice } = setup()
    vi.mocked(isRunDismissed).mockReturnValue(true)
    const count = await slice.attachAlgorithmProductOverlays([], 'cat-1', 'run-1')
    expect(count).toBe(0)
    expect(materializeWorkflowMapLayers).not.toHaveBeenCalled()
  })
  ...
  it('按 runId 组内绑定各产品成员', async () => {
    const { slice, calls } = setup()
    const { members } = setupGroup(slice, { runId: 'run-1', tags: ['SM', 'VOD'] })
    mockMaterialize([
      { overlay_layer_id: 'ov-sm', title: 'SM', product_tag: 'SM' },
      { overlay_layer_id: 'ov-vod', title: 'VOD', product_tag: 'VOD' },
    ])
    const count = await slice.attachAlgorithmProductOverlays([], 'cat-src', 'run-1')
    expect(count).toBe(2)
  ...
  it('已有同 overlay 游离层：刷新 timeList 并剪除 ready 的 in-flight 键', async () => {
    ...
    slice.setJobLayers([
      makeJob({ jobId: 'run-1', catalogId: 'cat-1', inFlightTimeKeys: ['20240501'] }),
    ])
    mockMaterialize([
      {
  ...
  it('OMEGA_BLOCK 游离层 + 组内占位：并入组并移除游离条目', async () => {
    const { slice } = setup()
    const { group, members } = setupGroup(slice, { runId: 'run-1', tags: ['OMEGA'] })
    ...
    mockMaterialize([{ overlay_layer_id: 'ov-om2', title: 'OMEGA_BLOCK', product_tag: 'OMEGA_BLOCK' }])
    await slice.attachAlgorithmProductOverlays([], 'cat-src', 'run-1')
    expect(members[0]!.importedRaster?.overlayLayerId).toBe('ov-om2')
    expect(members[0]!.dataState).toBe('imported')
    expect(activeLayers.map((l) => l.instanceId)).not.toContain(orphan.instanceId)
    expect(group.memberInstanceIds).not.toContain(orphan.instanceId)
  })

  it('OMEGA_BLOCK 无组时并入任意 OMEGA 占位', async () => {
    const { slice } = setup()
    const placeholder = makeLayer({ name: 'OMEGA', runGroupProductTag: 'OMEGA' })
    activeLayers.push(placeholder)
    vi.mocked(extractOverlayImportsFromResultRefs).mockReturnValue([
      { overlayLayerId: 'ov-om', title: 'OMEGA_BLOCK', productTag: 'OMEGA_BLOCK' },
    ])
    const count = await slice.attachAlgorithmProductOverlays(
      [{ title: 'x' }] as Parameters<typeof extractOverlayImportsFromResultRefs>[0],
      'cat-src',
    )
    expect(count).toBe(1)
  ...
  it('优先绑定到 wf-out 输出目录对应活跃图层', async () => {
    ...
    vi.mocked(extractOverlayImportsFromResultRefs).mockReturnValue([
      { overlayLayerId: 'ov-a', title: 'Algorithm Map Layer: SM', productTag: 'SM' },
    ])
    const count = await slice.attachAlgorithmProductOverlays(
  ...
  it('无匹配时新增游离图层并挂入运行组', async () => {
    ...
    mockMaterialize([{ overlay_layer_id: 'ov-new', title: 'SM', product_tag: 'SM' }])
    const count = await slice.attachAlgorithmProductOverlays([], 'cat-src', 'run-9')
    expect(count).toBe(1)
    const added = activeLayers.find((l) => l.importedRaster?.overlayLayerId === 'ov-new')
  ...
  it('running 空产物清除残留空态横幅', async () => {
    ...
    slice.setJobLayers([makeJob({ jobId: 'run-r', catalogId: 'cat-1', status: 'running' })])
    mockMaterialize([])
  ...
  it('409 / 不可物化冲突不写错误横幅', async () => {
    const { slice } = setup()
    slice.setJobLayers([makeJob({ jobId: 'run-f', catalogId: 'cat-1', status: 'failed' })])
    vi.mocked(materializeWorkflowMapLayers).mockRejectedValue(
      new Error('409 cannot materialize ExecutionStatus.failed'),
    )
    const count = await slice.attachAlgorithmProductOverlays([], 'cat-1', 'run-f')
    expect(count).toBe(0)
    expect(slice.workflowError.value).toBeNull()
  })
  ...
  it('其它物化失败写入 workflowError', async () => {
    const { slice } = setup()
    slice.setJobLayers([makeJob({ jobId: 'run-e', catalogId: 'cat-1', status: 'succeeded' })])
    vi.mocked(materializeWorkflowMapLayers).mockRejectedValue(new Error('boom'))
  ...
  it('已 dismiss 的 overlay 被过滤返回 0；forceBind 可绕过', async () => {
    ...
    mockMaterialize([{ overlay_layer_id: 'ov-sm', title: 'SM', product_tag: 'SM' }])
    vi.mocked(isOverlayDismissed).mockReturnValue(true)
    const filtered = await slice.attachAlgorithmProductOverlays([], 'cat-src', 'run-1')
    expect(filtered).toBe(0)
    const forced = await slice.attachAlgorithmProductOverlays([], 'cat-src', 'run-1', {
      forceBind: true,
    })
...
describe('reconcileOmegaBlockLayers', () => {
  ...
  it('游离层属于旧组时清理组成员关系，组空则删除组', () => {
    ...
    activeLayers.push(placeholder, orphan)
    slice.reconcileOmegaBlockLayers()
    expect(slice.runLayerGroups.value.map((g) => g.groupId)).not.toContain('grp-old')
  })
})
...
describe('reorderLayers', () => {
  ...
  it('锁定组成员不可拖出组外', () => {
    ...
    const group = makeGroup({ memberInstanceIds: [a.instanceId, b.instanceId] })
...
describe('createRunLayerGroup / bindRunIdToGroup', () => {
  it('创建占位成员与组，切换侧栏并选中首个成员', () => {
    ...
    expect(calls.setSelectedInstanceId).toHaveBeenCalledWith(created.memberInstanceIds[0])
    const members = activeLayers.filter((l) => l.runGroupId === created.groupId)
...
describe('cleanupUnproducedRunLayers', () => {
  it('无产物成员移除，有产物成员解锁并标（部分）', () => {
    const { slice, calls } = setup()
    const { group, members } = setupGroup(slice, { runId: 'run-1', tags: ['SM', 'VOD'] })
  ...
  it('全部成员无产物时组整体移除', () => {
    const { slice } = setup()
    const { group, members } = setupGroup(slice, { runId: 'run-2', tags: ['SM'] })
    slice.cleanupUnproducedRunLayers('run-2')
    expect(activeLayers.map((l) => l.instanceId)).not.toContain(members[0]!.instanceId)
...
describe('updateRunGroupFromJob / updateRunGroupForCatalog', () => {
  it('状态映射与进度壳文案', () => {
    ...
    slice.updateRunGroupFromJob('run-1', {
      ...
      message: '反演中',
      nodeProgress: [
        { nodeId: 'n1', nodeLabel: 'SF 反演', stage: 'inversion', progress: 40 },
      ],
    })
    expect(group.status).toBe('computing')
    ...
    slice.updateRunGroupFromJob('run-1', { status: 'succeeded', progress: 100, message: '' })
    expect(group.status).toBe('ready')
    slice.updateRunGroupFromJob('run-1', { status: 'cancelled', progress: 100, message: '' })
    expect(group.status).toBe('cancelled')
  })

  it('未知 runId 无操作', () => {
    const { slice } = setup()
    slice.updateRunGroupFromJob('run-none', { status: 'running', progress: 1, message: '' })
...
describe('reorderWithinRunGroup / moveRunGroupBlock', () => {
  ...
  it('组内有效调序重写 order', () => {
    ...
    const top = activeLayers.find((l) => l.instanceId === members[1]!.instanceId)!
    const bottom = activeLayers.find((l) => l.instanceId === members[0]!.instanceId)!
...
describe('findRunGroupByMember / findRunGroupById', () => {
  it('按成员与按 id 查找', () => {
    const { slice } = setup()
    const { group, members } = setupGroup(slice, { tags: ['SM'] })
    expect(slice.findRunGroupByMember(members[0]!.instanceId)?.groupId).toBe(group.groupId)
