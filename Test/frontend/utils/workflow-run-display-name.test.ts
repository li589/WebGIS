...
describe('workflow-run-display-name', () => {
  const summaries = [
    { workflow_id: 'omega_avg_daily_smap_online', name: 'SMAP ω 动态反演（在线）' },
    { workflow_id: 'omega_sf_fenkuai_smap_online', name: 'SMAP ω 分块反演（在线）' },
  ]
  ...
  it('isTechnicalRunTitle 识别 wf-run / run-group 占位', () => {
    ...
    expect(isTechnicalRunTitle('SMAP ω 动态反演（本地）')).toBe(false)
  })
  ...
  it('resolveRunGroupTitle 保留已配置中文组名并去掉计算中后缀', () => {
    expect(
      resolveRunGroupTitle({
        configuredTitle: 'SMAP ω 动态反演（本地） · 计算中',
      }),
    ).toBe('SMAP ω 动态反演（本地）')
  })
