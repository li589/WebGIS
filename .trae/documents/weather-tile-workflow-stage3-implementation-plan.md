# 天气图层 Workflow 瓦片化 Stage 3 实施计划（修订版）

## 1. 摘要（Summary）

针对当前天气图层“首次加载正常、随后忽然变短线/稀疏/消失，缩放后只剩少量流场线条”的问题，本计划完成 **Stage 3 接入与清理**，实现“纯 workflow 瓦片化”：

- **后端**：修复 `weather_tile_render` 节点，强制按 `zoom_to_resolution(z)` 固定分辨率输出，保证相邻瓦片网格行列一致、采样点对齐。
...
- **移动/缩放冲突**：视口变化时 generation++，过期 workflow 立即取消；BFS 外扩请求必须通过当前 generation 与允许集合校验，避免旧预取占用槽位。
...
| `Code/frontend/src/stores/weather-tile-manager.ts` | 已改造 | 具备全局 3 槽位、图层内 priority 0/1、generation 丢弃、BFS 外扩、429 退避，但存在两个并发计数 bug 尚未修复。 |
...
- **具体表现**：`resolveAllWeatherOverlayStates` 中 `const renderHint = layer.jobLayer?.mapLayerPayload?.renderHint`，tile manager 路径下 `jobLayer` 为空或没有 `renderHint`，导致天气图层被 `continue` 跳过，完全不渲染。
...
- **修复方式**：节点内直接调用 `OpenMeteoClient.fetch_grid_forecast(..., resolution=zoom_to_resolution(z))`，不再经过 `_fetch_layer_grid_data`。
...
**根因 E：`weather-tile-manager.ts` 并发计数 bug**
- `submitTile` 在“提交后 generation 过期”分支直接 `return`，未执行 `finally`，导致 `activeFetchCount` 泄漏；
- `submitTile` 在 429 分支中先递减 `activeFetchCount`，再 `return`，随后 `finally` 再次递减，导致计数被双重扣减。
...
import { buildDefaultWeatherRenderHint, isRealtimeWeatherLayerId } from './map/weather-render'
...
**改造内容**：
- 修复 `submitTile` 的 `activeFetchCount` 泄漏：
  - “提交后 generation 过期”分支在 `return` 前必须递减 `activeFetchCount` 并调用 `drainQueue()`；
  - 429 分支不再自行递减 `activeFetchCount`，仅设置退避时间并重新入队，由 `finally` 统一释放。
