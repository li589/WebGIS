...
radon_cmd = [
    ...
    "-i", "Env,venv,.venv,node_modules,.git,__pycache__,build,dist",
    "-e", "*/migrations/*,*/tests/*",
    "-a", "-s", "-j", "-o", "SCORE"
]

print(f"执行 radon 命令中......")
...
print("=" * 70)
file_totals = [(sum(comps), len(comps), max(comps), path) for path, comps in file_complexity.items()]
...
for i, (total_comp, func_count, max_comp, path) in enumerate(file_totals[:20], 1):
    ...
    print(f"  {i:>3d}. {total_comp:>8d}    {func_count:>4d}    {max_comp:>6d}    {short_path}")
...
print(f"   原始文件: {get_file_size(source_size_bytes)} → 精简后: 只有核心数据({get_file_size(out_size_bytes)})")
