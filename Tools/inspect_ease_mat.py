    ...
    for k, v in m.items():
        if isinstance(v, np.ndarray):
            if v.dtype.kind in ('U', 'S'):
                # String array (CRS)
                print(f"    {k}: shape={v.shape}, dtype={v.dtype}, value={str(v.ravel()[0]) if v.size > 0 else 'empty'}")
            elif v.size <= 16:
                # Small numeric array (Transform, Resolution)
                print(f"    {k}: shape={v.shape}, dtype={v.dtype}, value={v.ravel().tolist()}")
            else:
                print(f"    {k}: shape={v.shape}, dtype={v.dtype}, range=[{np.nanmin(v):.4f}, {np.nanmax(v):.4f}]")
...
print(f"  Global grid: 1692 rows x 3600 cols")
print(f"  Global width: {3600 * 9000.879 / 1000:.1f} km")
print(f"  Global height: {1692 * 9000.879 / 1000:.1f} km")
print(f"  Data shape (1624, 3856):")
print(f"    width: {3856 * 9000.879 / 1000:.1f} km ({3856 * 9000.879 / 40075000 * 360:.1f} deg at equator)")
