...
def check_omega_mat():
    ...
    try:
        try:
            ...
            print(f"MATLAB v5/v6 格式 (scipy.io)")
        except Exception:
            import h5py
            with h5py.File(f, "r") as h5:
                data = {k: v for k, v in h5.items()}
            print(f"MATLAB v7.3 格式 (h5py)")
        ...
        for d, f in tif_files[:5]:
            ...
            try:
                with rasterio.open(f) as ds:
                    ...
                    print(f"  Sample data (5x5): min={data.min():.2f} max={data.max():.2f} mean={data.mean():.2f}")
            except Exception as e:
                print(f"  错误: {e}")
    except ImportError:
    ...
    for d in sorted(ROOT.iterdir()):
        if d.is_dir():
            try:
                ...
                print(f"  {d.name:30s}  {file_count:8d} 文件  {size/1024/1024/1024:.2f} GB")
            except Exception as e:
