...
| 02 | `02-架构设计/` | 后端架构、空间表、工作流编辑器、工程决策纪要、架构图、图层平台结构化框架 |
| 03 | `03-规范协议/` | 技术栈/规范、PRD、命名、工作流节点、数据源与契约说明 |
| 04 | `04-执行部署/` | **[部署与再开发指南](04-执行部署/部署与再开发指南.md)（首选入口**：首次部署 SOP / 三态启停 / 验收 / 故障速查 / 改 X 跑 Y / 避坑清单）、本地联调环境、交付清单、[部署手册与硬约束清单](04-执行部署/部署手册与硬约束清单.md)（形态决策 / 硬约束 / 容器化方案）、[Win10 交接部署与三态启动方案](04-执行部署/Win10-交接部署与三态启动方案.md)（三态支持矩阵 / Docker 自定义 / 交接作业指引） |
| 05 | `05-专题研究/` | 天气渲染、omega_sf 反演、数据集目录、命名研究、其它专题 |
| 06 | `06-代码审查/` | 各轮代码审查报告与问题追踪 |
| 07 | `07-工程保障/` | 预启动检查、工程收口、功能验证、UI/底图修复、[联调缓存与生效边界](07-工程保障/联调缓存与生效边界.md)、[Agent 多配置档](07-工程保障/agent-profiles.md)、[运维手册](07-工程保障/运维手册.md)（启动故障/端口/事故记录） |
...
- **`08-HTML报告/frontend-refactoring-summary/`** — 前端 UI 重构总结报告（P0 面板容器收敛 / P1 大型组件拆分 / P2 设计系统对齐 / P3 无障碍审计 / 性能验证）
...
| 启动故障/端口 | 8080 被吞→WinNAT 排除段；环回被丢→安全软件 WFP；占用 vs 被保留的判定 | `07-工程保障/运维手册.md` |
