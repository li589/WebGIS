...
1. `/config/*` 写操作与敏感读需会话 Cookie、用户 API Token 或 `backend_auth` 服务密钥（`X-API-Key`）；RBAC 三角色：`admin`（全权限）/ `standard`（可读写工作流，不可改高危配置）/ `demo`（只读 + 受控数据传输），配置管理端点仅 `admin` 可写。dev 旁路仅 loopback，或 `BACKEND_DEV_AUTH_BYPASS`。
