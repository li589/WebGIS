...
> **状态（2026-08-04 晚）**  
> - 算法离线验证与 Matlab 对照：**已完成**  
> - UI 人工闭环：**已完成**（证据 `2026-08-04-fy-smap-ui-closed-loop.md`，run `run-d1f167fbaec4`）  
> - 刷新后图层：`workspace-persist` 恢复曾添加层属预期  
> - 残留：更大样本条带目视；导出按钮点击偶发被 map canvas 拦截
...
- [x] **服务启动**: `launch.py start` / status 全绿（无 dotenv 错误）
- [x] **图层管理**: 空态 / persist 恢复行为符合预期
- [x] **时间轴**: 无 FY/SMAP 选中时为 0–23 小时刻度
- [x] **工作流运行**: `run-d1f167fbaec4` succeeded
- [x] **输出对比**: runtime 块级 `YYYYMMDD_YYYYMMDD.mat` 已落盘（≈47，含既有拷贝）
- [x] **UI 显示**: SM / VOD / OMEGA 独立图层（更大样本条带目视可选加强）
- [x] **时间切换**: 2025-12-27 → 12-28 可用
- [ ] **数据导出**: 按钮「导出 PNG / GeoTIFF」可见；自动化点击被 map 拦截，待人工点一次确认
