...
> 各 AI 工具（Cursor / Trae / Copilot / Qoder / WorkBuddy）的规则文件保留在原路径，但只作**指针**，完整约定见本目录的 `rules/`。
...
| `memory/` | AI 记忆 / 历史上下文：`archive/`（原 `.trae/documents/*` ~72 份历史计划与对话） |
...
| 工具 | 做法 |
|------|------|
| Cursor | 根目录 `.cursorignore` 含 `!.ai/` 与 `!.ai/**`，覆盖 gitignore 排除（改后可在 Indexing 里 Refresh） |
